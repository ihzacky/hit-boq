...
class BoqWorkUnitLine(models.Model):
    ...
    # work_unit_line_price_final = fields.Monetary(string='Harga Final Pekerjaan', currency_field='currency_id', compute='_compute_work_unit_line_price_final')
    ...
    others_price_after_margin_final = fields.Monetary(string='Others Price After Margin', currency_field='currency_id', compute='_compute_components_price_after_margin_final')
    ...
    # --------------------------------------------------------------------------
    # after margin
    @api.depends('material_base_price', 'service_base_price', 'others_base_price', 'boq_root_id.material_margin', 'boq_root_id.installation_margin')
    def _compute_components_price_after_margin(self):
        for record in self:
            # Initialize values to prevent missing assignments
            record.material_base_price_after_margin = 0.0
            record.service_base_price_after_margin = 0.0
            record.others_base_price_after_margin = 0.0

            # Get margin values with fallback to 1.0
    ...
    @api.depends('material_base_price_after_margin', 'service_base_price_after_margin', 'others_base_price_after_margin', 'work_unit_line_quantity')
    def _compute_components_price_after_margin_final(self):
        for record in self:
            ...
            record.others_price_after_margin_final = 0.0
    ...
    def _get_work_unit_components(self):
        for record in self:
            # record.work_unit_line_code = f"{record.work_unit_id.work_unit_code}.{record.sequence}"
            record.work_unit_line_code = f"{record.work_unit_id.work_unit_code}"
            # record.work_unit_line_name = f"{record.work_unit_id.work_unit_name} - {record.sequence}"
